...
- The audit opens an explicitly visible interactive window and requests real foreground focus before measuring Frame Generation. Both Unreal and the NVIDIA SDK independently suspend FG for background windows, so overriding an engine focus flag is insufficient. The user-assisted foreground checks pass and record actual focused and generated-frame sample counts; no FPS multiplier is fabricated. Normal play retains ordinary focus behavior.
